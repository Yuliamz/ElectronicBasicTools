...
public class DrawPLA_DNF extends DrawPLA {
    ...
    public void drawOneAND(GraphicsContext gc, int i, int j) {
    ...
    public void drawANDs(GraphicsContext gc) {
        ...
        for (int k = 0; k < ExpressionDrawerParser.getAl().size(); k++) {
            if ((deleteEmpty(ExpressionDrawerParser.getAl().get(k)) >= 2)) {
                drawOneAND(gc, i, 60 + k * 30);
            }
        }
    }

    public void drawOneOR(GraphicsContext gc, int i, int j, int k) {
    ...
    public void drawORs(GraphicsContext gc) {
        getOutputGatexpos();
        drawOneOR(gc, getXposition(), 40, super.conns_OutputGate);
    }
